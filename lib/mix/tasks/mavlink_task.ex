defmodule Mix.Tasks.Mavlink do
    ...
    case parse_mavlink_xml(input) do
      ...
      %{version: version, dialect: dialect, enums: enums, messages: messages} ->
        ...
        defmodule Mavlink do
          ...
          @doc "helper function to unpack bitmasks"
          def unpack_bitmask(value, enum, acc \\\\ MapSet.new(), pos \\\\ 1) do
            case {decode(enum, pos), (value &&& pos) != 0, } do
              {not_atom, _} when not is_atom(not_atom) ->
                acc
              {entry, true} ->
                unpack_bitmask(value, enum, MapSet.put(acc, entry), pos <<< 1)
              {_, false} ->
                unpack_bitmask(value, enum, acc, pos <<< 1)
            end
          end
  ...
  defp field_type(%{enum: enum, display: :bitmask}) when enum != "", do: "MapSet.t(Mavlink.#{enum})"
  defp field_type(%{enum: enum}) when enum != "", do: "Mavlink.#{enum}"
