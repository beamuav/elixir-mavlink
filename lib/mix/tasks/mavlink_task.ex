defmodule Mix.Tasks.Mavlink do
    ...
    case parse_mavlink_xml(input) do
      ...
      %{version: version, dialect: dialect, enums: enums, messages: messages} ->
        ...
        defmodule Mavlink do
          ...
          @spec crc_extra(message_id) :: 0..255
          #{message_code_fragments |> map(& &1.crc_extra) |> join("") |> trim}
  ...
  defp get_message_code_fragments(messages, _enums) do
    for message <- messages do
      ...
      unpack_binary_pattern = wire_order |> map(& &1.name <> "_f::" <> type_to_binary(&1.type, &1.ordinality)) |> join(",")
      unpack_struct_fields = message.fields |> map(& &1.name <> ": " <> &1.name <> "_f") |> join(", ")
      crc_extra = message |> calculate_message_crc_extra
      %{
        crc_extra:
          """
            def crc_extra(#{message.id}), do: #{crc_extra}
          """,
        unpack:
          """
            def unpack(#{message.id}, <<#{unpack_binary_pattern}>>), do: %Mavlink.#{module_name}{#{unpack_struct_fields}}
          """,
        module:
          """
  ...
  defp type_to_binary("char", n), do: "binary-size(#{n})"
  defp type_to_binary("uint8_t", n), do: "binary-size(#{n})"
  defp type_to_binary("int8_t", n), do: "binary-size(#{n})"
  defp type_to_binary("uint16_t", n), do: "binary-size(#{n * 2})"
  defp type_to_binary("int16_t", n), do: "binary-size(#{n * 2})"
  defp type_to_binary("uint32_t", n), do: "binary-size(#{n * 4})"
  defp type_to_binary("int32_t", n), do: "binary-size(#{n * 4})"
  defp type_to_binary("uint64_t", n), do: "binary-size(#{n * 8})"
  defp type_to_binary("int64_t", n), do: "binary-size(#{n * 8})"
  defp type_to_binary("float", n), do: "binary-size(#{n * 4})"
  defp type_to_binary("double", n), do: "binary-size(#{n * 8})"
  defp type_to_binary("char"), do: "integer-size(8)"
  defp type_to_binary("uint8_t"), do: "integer-size(8)"
  defp type_to_binary("int8_t"), do: "signed-integer-size(8)"
  defp type_to_binary("uint16_t"), do: "little-integer-size(16)"
  defp type_to_binary("int16_t"), do: "little-signed-integer-size(16)"
  defp type_to_binary("uint32_t"), do: "little-integer-size(32)"
  defp type_to_binary("int32_t"), do: "little-signed-integer-size(32)"
  defp type_to_binary("uint64_t"), do: "little-integer-size(64)"
  defp type_to_binary("int64_t"), do: "little-signed-integer-size(64)"
  defp type_to_binary("float"), do: "little-signed-float-size(32)"
  defp type_to_binary("double"), do: "little-signed-float-size(64)"
